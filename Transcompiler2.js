...
export default function LogMonitor({ title, logGenerator, interval = 2000, logColor = "text-green-300" }) {
  const [logs, setLogs] = useState([]);

  useEffect(() => {
    const id = setInterval(() => {
      setLogs((prev) => [...prev.slice(-49), logGenerator()]);
    }, interval);
    return () => clearInterval(id);
  }, [logGenerator, interval]);

  return (
    <Card className="mt-4">
      <CardContent className={`p-4 bg-gray-900 ${logColor} font-mono text-sm rounded-xl`}>
...
<LogMonitor
  title="Directive Execution Logs"
  logGenerator={() => {
    ...
    return `[Directive] ${directive} → ${status}`;
  }}
...
export function LogProvider({ children }) {
  ...
  const addLog = (log) => setLogs((prev) => [...prev.slice(-49), log]);
...
const LogMonitor = React.memo(({ title, logs, onClear }) => {
  return (
    <Card className="mt-4">
...
return (
  <div>
    <select onChange={(e) => setFilter(e.target.value)} className="p-2 bg-gray-800 text-white rounded">
    ...
    <div className="mt-2 h-32 overflow-auto">
      {filteredLogs.map((log, i) => (
        <div key={i} className={`text-${log.severity === "error" ? "red" : log.severity === "warn" ? "yellow" : "green"}-300`}>
          {log.message}
        </div>
      ))}
    </div>
